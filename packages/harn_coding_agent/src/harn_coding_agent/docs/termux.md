...
Harn runs on Android via [Termux](https://termux.dev/), a terminal emulator and Linux environment for Android.
...
Create `~/.harn/agent/AGENTS.md` to help the agent understand the Termux environment:
...
- **No native binaries**: Some optional native dependencies are unavailable on Android ARM64 and are skipped during installation
...
If pip fails, try upgrading pip:
```bash
