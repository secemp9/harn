> harn can create themes. Ask it to build one for your setup.
...
Harn loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.harn/agent/themes/*.json`
- Project: `.harn/themes/*.json`
- Packages: `themes/` directories or `harn.themes` entries in `pyproject.toml`
...
On first run, harn detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.harn/agent/themes
vim ~/.harn/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, harn reloads it automatically for immediate visual feedback.
...
Harn uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, harn falls back to the nearest approximation.
